...
REPORT_VERSION = "annual-report-free-note-v2"
...
<label class="note-label">Your Note<textarea name="raw_note" placeholder="Write freely: what you saw, what it might mean, alternatives, questions, context, or evidence needs."></textarea></label>
<button type="button" class="save-note">Save</button><button type="button" class="cancel-edit" hidden>Cancel edit</button><div class="persisted-records"><strong>Persisted notes</strong><div class="record-list"><p>Connect through the localhost Observatory application to load records.</p></div></div></form>"""
...
const clearForm=form=>{{form.reset();delete form.dataset.editingId;form.querySelector('.save-note').textContent='Save';form.querySelector('.cancel-edit').hidden=true;setState(form,'Unsaved draft');form.querySelector('.save-status').textContent='';}};
const fillForm=(form,record)=>{{form.raw_note.value=record.raw_note??record.observation_text??'';form.dataset.editingId=record.observation_id;form.querySelector('.save-note').textContent='Save edit as revision '+(record.revision+1);form.querySelector('.cancel-edit').hidden=false;setState(form,'Editing persisted revision '+record.revision+' — unsaved changes');}};
const line=(parent,label,text)=>{{const p=document.createElement('p');const strong=document.createElement('strong');strong.textContent=label+': ';p.append(strong,document.createTextNode(text||'—'));parent.append(p);}};
const machineReading=record=>{{const details=document.createElement('details');details.className='derived-note';const summary=document.createElement('summary');summary.textContent='MACHINE READING OF YOUR NOTE · derived analysis v'+(record.derived_analysis?.analysis_version??'n/a');details.append(summary);if(!record.derived_analysis){{line(details,'Status','Legacy structured record; no derived analysis exists. Original fields remain preserved below.');return details;}}const analysis=record.derived_analysis;(analysis.extracted_claims||[]).forEach(claim=>{{const card=document.createElement('article');card.className='claim-card';line(card,'Extracted statement',claim.extracted_statement);line(card,'Classification',claim.claim_type_label);line(card,'Evidence status',claim.evidence_status);line(card,'Basis',claim.semantic_or_deterministic_basis);(claim.supporting_measurements||[]).forEach(metric=>line(card,'Bound measurement',metric.metric+' = '+JSON.stringify(metric.value)+' · '+metric.formula_or_rule+' · '+metric.artifact_path+' # '+metric.artifact_sha256));line(card,'Limitation',claim.limitation);if(claim.interpretation_boundary)line(card,'Interpretation boundary',claim.interpretation_boundary);if((claim.unresolved_evidence||[]).length)line(card,'What remains unresolved',claim.unresolved_evidence.join(' | '));details.append(card);}});line(details,'Machine-derived retrieval metadata',(analysis.machine_derived_retrieval_metadata||[]).join(', ')||'none');line(details,'Method',analysis.analysis_method);line(details,'Product boundary',analysis.limitations);return details;}};
const legacyCard=record=>{{const populated=[['Observation',record.observation_text],['Hypothesis',record.hypothesis],['Counter-Hypothesis',record.counter_hypothesis],['Evidence Needed',record.evidence_needed],['Tags',(record.tags||[]).join(', ')],['Confidence',record.confidence==null?'':String(record.confidence)],['Author',record.author]].filter(x=>x[1]);if(!populated.length)return null;const details=document.createElement('details');details.className='legacy-fields';const summary=document.createElement('summary');summary.textContent='Original structured human fields (preserved losslessly)';details.append(summary);populated.forEach(x=>line(details,x[0],x[1]));return details;}};
const renderRecords=(form,records)=>{{const list=form.querySelector('.record-list');list.replaceChildren();if(!records.length){{const p=document.createElement('p');p.textContent='No persisted notes for this target.';list.append(p);return;}}records.forEach(record=>{{const article=document.createElement('article');article.className='persisted-record';const heading=document.createElement('strong');heading.textContent='Persisted · '+record.updated_at+' · revision '+record.revision;const text=document.createElement('pre');text.className='raw-note';text.textContent=record.raw_note??record.observation_text??'';const meta=document.createElement('p');meta.className='record-meta';meta.textContent='Raw human artifact · author: '+(record.author||'unspecified')+' · created: '+record.created_at;const edit=document.createElement('button');edit.type='button';edit.textContent='Edit this note';edit.onclick=()=>fillForm(form,record);article.append(heading,text,meta);const legacy=legacyCard(record);if(legacy)article.append(legacy);article.append(machineReading(record),edit);list.append(article);}});}};
...
document.querySelectorAll('.human-form').forEach(form=>{{form.addEventListener('input',()=>{{const prefix=form.dataset.editingId?'Editing persisted note — ':' ';setState(form,prefix+'Unsaved draft');}});form.querySelector('.cancel-edit').onclick=()=>clearForm(form);form.querySelector('.save-note').onclick=async()=>{{if(!serverMode)return;const payload={{observation_id:form.dataset.editingId||undefined,pair_id:form.dataset.pair,quarter:form.dataset.period,target_scope:form.dataset.scope,chart_type:form.dataset.chart||null,raw_note:value(form,'raw_note')}};try{{const response=await fetch('/api/observations',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(payload)}});const data=await response.json();if(!response.ok)throw new Error(data.error||'Save failed');clearForm(form);setState(form,'Persisted revision '+data.record.revision,'persisted');form.querySelector('.save-status').textContent=' Saved verbatim '+data.record.updated_at;await loadRecords(form);}}catch(error){{setState(form,'Save failed','error');form.querySelector('.save-status').textContent=' '+error.message;}}}};if(!serverMode){{form.querySelector('.save-note').disabled=true;form.querySelector('.save-status').textContent=' Static view: saving requires observatory-serve.';}}else{{loadRecords(form).catch(error=>{{setState(form,'Load failed','error');form.querySelector('.save-status').textContent=' '+error.message;}});}}}});
