...
TAGS = ("synchronous", "possible catch-up", "possible reversal", "persistent divergence", "stock-specific event", "unclear")
GUIDANCE = {
    "return_overlay": ("Two aligned daily log-return series, r(t)=ln(P(t)/P(t-1)), plotted on their common trading dates.", ("Inspect whether moves share sign and timing.", "Note volatility changes, isolated spikes, and clustering.", "Treat apparent timing differences as questions, not causal evidence.")),
    "normalized_price": ("Each quarter's qfq close divided by its first common close; both paths begin at 1.", ("Inspect sustained separation or convergence.", "Note whether gaps close through catch-up, reversal, or both.", "Check whether the visual changes within the quarter.")),
    "return_scatter": ("Paired contemporaneous daily log returns; x is the first ticker and y is the second.", ("Inspect orientation, dispersion, asymmetry, and outliers.", "Note whether extreme moves differ from the central cloud.", "Do not infer causality from slope or correlation.")),
    "lag_correlation": ("Pearson correlation of r_a(t) with r_b(t+lag) for each fixed lag -5…+5, using quarter-dated return observations.", ("Inspect whether lag 0 or an off-zero lag is visually largest.", "Compare positive- and negative-lag profiles.", "Treat differences as descriptive and sample-dependent.")),
    "event_response": ("Cumulative response log returns around source events with |log return|>=3%; fixed ±5 common-session windows may cross quarter boundaries.", ("Inspect dispersion across events and the descriptive mean path.", "Compare event paths without assigning causation.", "Consider sourced company and industry context as alternative explanations.")),
}
...
def _context_sections(quarter: str, company: list[dict], events: list[dict]) -> str:
    ...
    if company_rows:
        rows = "".join(f"<tr><td>{html.escape(str(r['ticker']))}</td><td>{html.escape(str(r['attribute']))}</td><td>{html.escape(str(r['value']))} {html.escape(str(r.get('unit','')))}</td><td>{html.escape(str(r['as_of_date']))}</td><td><a href=\"{html.escape(str(r['source_url']))}\">{html.escape(str(r['source']))}</a></td></tr>" for r in company_rows)
        company_html = f"<table class=\"context-table\"><tr><th>Ticker</th><th>Attribute</th><th>Value</th><th>As of</th><th>Source</th></tr>{rows}</table>"
    else:
        company_html = "<p>No sourced quarter-level company attributes were supplied. Nothing is inferred from price data.</p>"
    if event_rows:
        rows = "".join(f"<tr><td>{html.escape(str(r['date']))}</td><td>{html.escape(str(r['scope']))}</td><td>{html.escape(str(r['description']))}</td><td><a href=\"{html.escape(str(r['source_url']))}\">{html.escape(str(r['source']))}</a></td></tr>" for r in event_rows)
        event_html = f"<table class=\"context-table\"><tr><th>Date</th><th>Scope</th><th>Description (no causal claim)</th><th>Source / provenance</th></tr>{rows}</table>"
    else:
...
<label class="confidence">Optional confidence (0–1): <input name="confidence" type="number" min="0" max="1" step="0.01"></label><button type="button" class="save-note">Save structured exploratory note</button><span class="save-status" aria-live="polite"></span></form>"""
...
def _annual_html(output: Path, config: RunConfig, year_label: str, quarters: list[str], metadata: dict[str, dict], provider_id: str, provider_version: str, company: list[dict], events: list[dict]) -> str:
    ...
    for label, filename, kind in PLOT_ROWS:
        cells = []; explanation, prompts = GUIDANCE[kind]
        for quarter in quarters:
            source = f"../quarters/{quarter}/{filename}"; guidance = "".join(f"<li>{html.escape(p)}</li>" for p in prompts)
            cells.append(f"<td><a href=\"{source}\" target=\"_blank\" rel=\"noopener\"><img src=\"{source}\" alt=\"{quarter} {html.escape(label)}\"></a><section class=\"guide\"><h4>What this chart represents</h4><p>{html.escape(explanation)}</p><h4>Traditional visual features to inspect</h4><ul>{guidance}</ul></section>{_machine_html(kind, metadata[quarter]['machine'], config.ticker_a, config.ticker_b)}{_human_form(pair, quarter, kind)}</td>")
...
<script>const KEY='observatory-human-notes:{pair}';const read=()=>JSON.parse(localStorage.getItem(KEY)||'[]');const write=x=>localStorage.setItem(KEY,JSON.stringify(x));document.querySelectorAll('.human-form').forEach(f=>{{const old=read().find(x=>x.id===f.dataset.recordId);if(old){{f.observation.value=old.observation_text||'';f.hypothesis.value=old.hypothesis||'';f.alternative.value=old.alternative_explanation||'';f.confidence.value=old.confidence??'';f.querySelectorAll('[name=tags]').forEach(x=>x.checked=(old.tags||[]).includes(x.value));}}f.querySelector('.save-note').onclick=()=>{{const c=f.confidence.value;const record={{id:f.dataset.recordId,record_type:'exploratory_human_note',pair:f.dataset.pair.split('/'),period:f.dataset.period,chart_type:f.dataset.chart,observation_text:f.observation.value,tags:[...f.querySelectorAll('[name=tags]:checked')].map(x=>x.value),hypothesis:f.hypothesis.value,alternative_explanation:f.alternative.value,confidence:c===''?null:Number(c),timestamp:new Date().toISOString(),prohibited_uses:['training labels','formal classifications','pair-selection inputs','trading signals']}};const all=read().filter(x=>x.id!==record.id);all.push(record);write(all);f.querySelector('.save-status').textContent='Saved '+record.timestamp;}};}});document.getElementById('export-notes').onclick=()=>{{const blob=new Blob([JSON.stringify({{schema_version:'1.0',records:read()}},null,2)],{{type:'application/json'}});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='human_observations_{pair.replace('/','_')}.json';a.click();URL.revokeObjectURL(a.href);}};document.getElementById('import-notes').onchange=e=>{{const reader=new FileReader();reader.onload=()=>{{const data=JSON.parse(reader.result);write(data.records||[]);location.reload();}};reader.readAsText(e.target.files[0]);}};</script></body></html>"""
